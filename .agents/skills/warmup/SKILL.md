...
AGENTS.md names a different common-rules checkout.
...
`RULES/bin/warmup --project . --queue --state .Codex/warmup/last.json`
...
- **`--state .Codex/warmup/last.json`** saves this run's state, so `/reheat`
  has a baseline to compare against later in the session. `.Codex/warmup/`
...
ledger JSON → the latest checkpoint → common-rules' Codex-workflow.md. Open a
...
- **Ruflo is mandatory.** The card shows where the CLI is; warm-up never runs
  ...
  **Never register Ruflo's full MCP tool set for this reason.** `bin/ruflo-item`
  is a CLI wrapper, not an MCP client — it shells out to the `Codex-flow`/
  ...
  MCP tool-definition tokens: nothing here needs the 333-tool `Codex-flow`
  MCP server on this project's `.Codex.json`/`.mcp.json` at all (proposal 19's
  ...
  session's `~/.Codex.json` registers it anyway (for another project, or a
...
- **`page changed since last publish: docs/proposals/tracker/index.html → <url>`**
  ...
  2. Publish that committed page file with your Artifact tool **to the same
     ...
     by hand (`Codex-workflow.md`, "Talking to the user", proposal 22 T-06 —
...
1. `RULES/bin/warmup --project . --migrate --dry-run` first. It writes nothing
   ...
   under the next free proposal number, supersede rules, point AGENTS.md at
...
3. AGENTS.md is the sponsor's: land will not land a branch that touches it.
