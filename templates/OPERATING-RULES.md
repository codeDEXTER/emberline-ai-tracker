...
- **The ledger is the plan of record, updated in place.** `{{PLAN_LEDGER}}`
  is the plan; its page is generated by `tracker render`, never hand-edited.
  Never a new plan document: a revision adds rows and corrects numbers, and
  never scraps old content -- superseded measurements stay, marked as
  superseded.
