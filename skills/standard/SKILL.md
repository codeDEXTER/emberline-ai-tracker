...
`RULES` is `/Users/aashish/apps/common-rules`. Every project under `/Users/aashish/apps` follows the same steps, so every session speaks the same language: one ledger shape, one card, one proposal shape, one way to ask another session for something.
