...
- You keep working until every item is `done` and every exit condition in
  ...
  committing the warm-up pointer block in CLAUDE.md (his file); installing `derecord --parent` on a start folder (`/Users/aashish/apps/PhotoVault`, `/Users/aashish/apps`); every edit to this repository beyond what he has directed; S-07 and S-08, which run in the PhotoVault sessions once he types `/standard` there. When you hit one, mark the item `blocked`,
